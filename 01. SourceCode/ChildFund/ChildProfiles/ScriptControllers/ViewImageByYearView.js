...
function DownLoadImageAll() {

    $('#divListImage input:checkbox[class=itemp]').each(function () {
        $(this).attr('checked', "checked");
    });
    setTimeout(function () { DownLoadImage(); }, 300);
}
function DownLoadImage() {
    ...
    if (listIdChecked.length > 0) {
        ...
        modelAction.UserUploadId = '';
        $('#loader_id').addClass("loader");
        document.getElementById("overlay").style.display = "block";
        $.ajax({
            url: "/ProfileNew/DownImageChild/",
            type: "POST",
            data: modelAction,
            success: function (data) {
                if (data.ok === true) {
                    ...
                    link.download = year !== null ?'Image_'+ year + '.zip' : 'Download.zip';
...
function Delete() {
    modelAction.ImageId = listIdChecked;
    $.post("/ProfileNew/DeleteImageChild", modelAction, function (result) {
