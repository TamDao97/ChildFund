...
var modelFamilyInfo = [];
function Update(isExportWord) {
    model.IsRemoteImg = IsRemoteImg;
    model.ProcessStatus = $('#hdStatus').val();
    model.IsExportWord = isExportWord;
    ...
    if (ValidateFormUpdate()) {
        ...
        $.ajax({
            url: "/ProfileNew/UpdateChildProfile",
            data: dataProfile,
            type: "POST",
            cache: false,
            processData: false,
            ...
            success: function (data) {
                if (data.ok === true) {
                    if (isExportWord) {
                        ...
                        sessionStorage.setItem("keyMess", "Cập nhật hồ sơ thành công!/Update profile successfully!");
...
function ValidateFormUpdate() {
    ...
    if (validateInfo !== 1 && validateInfo !== 2 && validateInfo !== 3) {
        ...
        if (validateInfo === 1) {
            toastr.error("Họ tên thành viên không được để trống!", { timeOut: 5000 }); return false;
        } else if (validateInfo === 3) {
            toastr.error("Sai định dạng năm!", { timeOut: 5000 }); return false;
        } else {
            toastr.error("Mối quan hệ không được để trống!", { timeOut: 5000 }); return false;
        }
    ...
    if (model.InfoDate === '') {
        toastr.error("Nhập ngày thu thập thông tin!", { timeOut: 5000 }); return false;
    } else if (model.TeachName === '') {
        toastr.error("Nhập tên cán bộ phụ trách!", { timeOut: 5000 }); return false;
    } else if (model.ProgramCode === '') {
        toastr.error("Nhập mã chương trình!", { timeOut: 5000 }); return false;
    } else if (model.ProvinceId === '') {
        toastr.error("Nhập tỉnh thành!", { timeOut: 5000 }); return false;
    } else if (model.DistrictId === '') {
        toastr.error("Nhập huyện!", { timeOut: 5000 }); return false;
    } else if (model.ChildCode === '') {
        toastr.error("Nhập mã số trẻ!", { timeOut: 5000 }); return false;
    }
    else if (model.ProgramCode.startsWith("199") === false && model.ProgramCode.startsWith("213") === false) {
        toastr.error("Mã chương trình phải bắt đầu là 199 hoặc 213!", { timeOut: 5000 }); return false;
    }

    else if (model.Name === '') {
        toastr.error("Nhập họ tên trẻ!", { timeOut: 5000 }); return false;
    }
    else if (model.DateOfBirth === '') {
        toastr.error("Nhập ngày sinh  trẻ!", { timeOut: 5000 }); return false;
    }
    else if (model.LeaningStatus === '') {
        toastr.error("Chọn tình trạng học của trẻ!", { timeOut: 5000 }); return false;
    }

    return true;
}

function GenInfoUpdate() {
    var idch = $('#tbLeaning').attr('name');
    var idHandicap = $('#handicap').attr('name');
    $('#tbLeaning input:checkbox').each(function () {
    ...
    model.Id = getUrlParameter('id');
    if (idch == '15') {
        var ClassInfo = $('#tbLeaning').attr('title');
        $('#classInfo').val(ClassInfo);
        $('#classInfo').prop('disabled', false);
    }
    if (idch == '16') {
        var ClassInfo = $('#tbLeaning').attr('title');
        $('#classInfo2').val(ClassInfo);
        $('#classInfo2').prop('disabled', false);
    }
}

function InitModel(DistrictId, WardId, VillageId, SchoolId) {
    $.post('/ProfileCategory/FamilyInfo?model=' + $('#familyInfoValue').val() + '&typeAction=edit', function (result) {
        $("#divfamilyInfo").html(result);
        CountGender(-1);
    });
    GenInfoUpdate();
    ChangeProvinceInit(DistrictId, WardId, VillageId, SchoolId);
    $('#tbnotLivingWithParentModel input:checked').each(function () {
        if ($(this).attr('value') == '01') {
            $('#tbnotLivingWithParentModel input').each(function () {
                if ($(this).attr('value') != '01' && $(this).attr('value') != '11') {
        ...
        if ($(this).attr('value') == '02') {
            $('#tbnotLivingWithParentModel input').each(function () {
                if ($(this).attr('value') == '04' || $(this).attr('value') == '06'
                    || $(this).attr('value') == '07' || $(this).attr('value') == '08'
                    || $(this).attr('value') == '09' || $(this).attr('value') == '01') {
                    $(this).prop('disabled', true);
                }
            });
        }

        if ($(this).attr('value') == '03') {
            $('#tbnotLivingWithParentModel input').each(function () {
                if ($(this).attr('value') == '01' || $(this).attr('value') == '05'
                    || $(this).attr('value') == '06' || $(this).attr('value') == '07'
                    || $(this).attr('value') == '08' || $(this).attr('value') == '10') {
                    $(this).prop('disabled', true);
                }
            });
        }
...
function ChangeProvince() {
    ...
    $.post('/Combobox/DistrictByUser?Id=' + comboboxid, function (result) {
...
function ChangeDistrict() {
    ...
    $.post('/Combobox/WardByUser?Id=' + comboboxid, function (result) {
        $("#wardId").html(result);
        ChangeWard();
    });
}
function ChangeProvinceInit(DistrictId, WardId, VillageId, SchoolId) {
    var comboboxid = $('#provinceId').val();
    $.post('/Combobox/DistrictByUser?Id=' + comboboxid, function (result) {
        $("#districtId").html(result);
        $("#districtId").val(DistrictId);
        ChangeDistrictInit(WardId, VillageId, SchoolId);
    });

}
function ChangeDistrictInit(WardId, VillageId, SchoolId) {
    var comboboxid = $('#districtId').val();
    $.post('/Combobox/WardByUser?Id=' + comboboxid, function (result) {
        $("#wardId").html(result);
        $("#wardId").val(WardId);
        ChangeWardInit(VillageId, SchoolId);
    });
}

function ChangeWardInit(VillageId, SchoolId) {
    var comboboxid = $('#wardId').val();
    $.post('/Combobox/GetVillageByWrad?Id=' + comboboxid, function (result) {
    ...
    $.post("/Combobox/SchoolCBB?id=" + comboboxid, function (result) {
