...
struct DateHelper {
    ...
    static func dateFromString(_ dateString: String, with format: String) -> Date? {
        let serverDateFormatter = DateFormatter(dateFormat: format)
        return serverDateFormatter.date(from: dateString)
    }
    
    static func formatYoutubeDate(youtubeDate: String) -> String {
        let dateFormatter = ISO8601DateFormatter()
        guard let date = dateFormatter.date(from: youtubeDate) else {
            return ""
        }
        let toDateFormatter = DateFormatter(dateFormat: DateFormatString.ddMMyyyy)
